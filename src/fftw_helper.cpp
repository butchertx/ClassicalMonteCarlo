...
std::vector<double> calc_correlation_no_wisdom(std::vector<double> state, int Lx, int Ly, int Lz) {
    int L = state.size();
    std::vector<double> result(L);
    fftw_complex* in, * out;
    fftw_plan p_forward, p_backward;
    ...
    p_forward = fftw_plan_dft_3d(Lx, Ly, Lz, in, out, FFTW_FORWARD, FFTW_ESTIMATE);
    p_backward = fftw_plan_dft_3d(Lx, Ly, Lz, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
    fftw_execute(p_forward);
    dagger_norm(out, in, L);
    fftw_execute(p_backward);
    for (int i = 0; i < L; ++i) {
