...
using namespace cmctype;

/*! \brief Enum for specifying a lattice type
*/
enum Lattice_type_t { CHAIN, TRIANGLE, KAGOME, DICE, HONEYCOMB, SQUARE, CUBIC, DIAMOND, DIAMOND111, DIAMONDCC, CGT };//require SQUARE is the last 2-D lattice in list
...
private:
	...
	double c_const = 0;//c-axis constant for 3-D lattices
...
public:
	...
	int get_neighbor_label(int origin, vec3<double> shift) {
		int result = origin;
		double dist = sqrt(shift * shift) - EPSILON;
		int dist_ind = std::lower_bound(distances.begin(), distances.end(), dist) - distances.begin();
		std::vector<int> neighbor_list = get_neighbors(origin, dist_ind);
