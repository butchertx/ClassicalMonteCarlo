...
Lattice::Lattice(Lattice_type_t lat_type_in, vec3<int> L_in, vec3<int> pbc_in)
	...
	case CGT:
		...
		c_const = 1.0;
		basis.push_back(vec3<double>(0.0, 0.0, 0.0));
		basis.push_back(vec3<double>(0.0, a_const, 0.0));

		a = vec3<vec3<double>>(vec3<double>(a_const * sqrt(3.0), 0.0, 0.0), vec3<double>(a_const * sqrt(3.0) / 2.0, a_const * 3.0 / 2.0, 0.0), vec3<double>(0.0, a_const, c_const));
		break;
	default:
